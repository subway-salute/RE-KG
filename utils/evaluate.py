...
def test_one_user(x):
    ...
    try:
        ...
        training_items = []
    # user u's items in the test set（正样本）
    user_pos_test = test_user_set[u]
    num_pos = len(user_pos_test)  # 正样本数量（通常为1）

    # 所有可能的物品集合
    all_items = set(range(0, n_items))
    # 负样本候选池：排除训练集已交互物品和测试集正样本（确保负样本有效性）
    negative_candidates = list(all_items - set(training_items) - set(user_pos_test))

    # 采样99个负样本（若候选池不足则全部选取，避免报错）
    num_neg = 99
    num_neg_to_sample = min(num_neg, len(negative_candidates))
    sampled_negatives = random.sample(negative_candidates, num_neg_to_sample)

    # 待排序物品集：采样的负样本 + 所有正样本（共 num_neg_to_sample + num_pos 个，通常为100个）
    test_items = sampled_negatives + user_pos_test
